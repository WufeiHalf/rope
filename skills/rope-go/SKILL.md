...
6. Review:
   - `Review: required` -> prefer harness preset `rope-reviewer` when `~/.config/rope/harness/<host>.json` maps it; else discover an available read-only review subagent type; if only non-specialized types are available, prefer `general-purpose` with explicit read-only instructions; if presets are missing, record `preset_missing` and continue; if no subagent tool or suitable type is available, record the degraded self-review reason in `tasks.md`
